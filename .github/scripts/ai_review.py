...
def should_use_claude(diff: str, has_important_label: bool, line_threshold: int = 0) -> bool:
    ...
    diff_lines = diff.count('\n')
    added_removed_lines = len([line for line in diff.split('\n') if line.startswith(('+', '-'))])
    
    if added_removed_lines > line_threshold:
        print(f"Using Claude due to large diff: {added_removed_lines} lines > {line_threshold} threshold", file=sys.stderr)
        return True
    
    print(f"Using o3-mini for small diff: {added_removed_lines} lines <= {line_threshold} threshold", file=sys.stderr)
    return False
...
if __name__ == "__main__":
    ...
    has_important_label = os.environ.get('HAS_IMPORTANT_LABEL', 'false').lower() == 'true'
    ...
    if should_use_claude(diff, has_important_label, line_threshold):
        ...
        model_comment = "This response was generated by GPT o3 mini."
