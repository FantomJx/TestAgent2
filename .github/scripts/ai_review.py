...
def create_openai_payload(model: str, prompt: str) -> Dict[str, Any]:
    ...
    # Use max_completion_tokens for o3-mini, max_tokens for other models including gpt-4.1-nano
...
def call_openai_api(api_key: str, payload: Dict[str, Any]) -> str:
    ...
    try:
        ...
        try:
            cost_tracker = CostTracker()
            cost_tracker.track_api_call(
                model=payload.get('model', 'gpt-4.1-nano'),
                response_data=data,
                call_type="review",
                context="Code review analysis"
            )
...
def should_use_claude(diff: str, has_important_label: bool, line_threshold: int = 0) -> bool:
    ...
    print(
        f"Using GPT-4.1 nano for small diff: {added_removed_lines} lines <= {line_threshold} threshold", file=sys.stderr)
    return False
